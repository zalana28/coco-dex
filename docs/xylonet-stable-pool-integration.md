...
CocoStablePool V1 is live on Arc Testnet as a Coco Native Stable Pool LP Beta and is documented separately in `coco-stable-pool-v1-spec.md` and `stable-pool-readiness.md`. It is unaudited, not routed, and indexed only through separate beta observability. The External Stable Pools panel is read-only and does not mean Coco owns or controls XyloNet or other external pools.
