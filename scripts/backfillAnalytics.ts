 ...
 *
 * Optional:
 *   BACKFILL_FROM_BLOCK  — default: 44170190
 *   BACKFILL_TO_BLOCK    — default: latest block
...
async function main() {
  ...
  console.log('\nBackfilling null timestamps on existing rows...')
  let updatedTotal = 0
  ...
  while (true) {
    ...
    const distinctBlocks = [...new Set(nullRows.map((r) => r.block_number as number))]
    ...
    console.log(`  Updated ${updatedTotal} rows so far...`)
  }

  console.log(`Timestamp backfill complete. Updated ${updatedTotal} rows.`)
}
